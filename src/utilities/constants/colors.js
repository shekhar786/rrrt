...
import { getAppId } from '../helperFunctions';
...
const commonColors = (appId = getAppId()) => {
    const color = {
        themeColor: colors.olive1
    };

    switch (appId) {
        case appTypes.yabalash.id: {
            color.themeColor = colors.olive1;
            break;
        }

        case appTypes.shilengae.id: {
            color.themeColor = colors.red1;
            break;
        }

        case appTypes.beault.id: {
            color.themeColor = colors.pink1;
            break;
        }
